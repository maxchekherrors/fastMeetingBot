...
exports.inviteAvailable = {
	...
	agree: async (ctx) => {
		...
		const agreedInvite = await Invite.findOneAndUpdate({
			$and: [
				{_id: agreedId},
		...
		const userInvite = await Invite.exists({
			$and: [
				{_id: inviteId},
				{agreedInvitations: agreedId},
			]
		});
		if (userInvite) {
			...
			Extra
				...
				.load({
					caption: `${
						...
						description
					}\n<b>Соглас${sex==='m'?'ен':'на'} на ваше приглашение!</b>`
				})

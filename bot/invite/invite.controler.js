...
exports.askLocation = (ctx) =>
	ctx.reply('Ok, now, send me your location and I find friends for you.',
		Extra
			.HTML()
...
exports.getInviteDescription = async (ctx) => {
	...
	return ctx.replyWithHTML('Nice description, you can send invite photo if you want. Or I will use your profile photo)',
		Extra
			.HTML()
			.markup((markup) => markup.resize()
				.keyboard([
...
exports.createInvite = async (ctx) => {
	...
	return ctx.replyWithHTML('Tell me about your invite your can write short description');
};
exports.getInvitePhoto = async (ctx) => {
	const {inviteId} = ctx;
	const photo = ctx.message.photo[0].file_id;
	await Invite.updateOne({_id: inviteId}, {photo});
	return ctx.replyWithHTML('Ouu, have a nice dickkkk, respect!');
};
exports.findFriends = async (ctx) => {
	const {userId, inviteId} = ctx;
	await ctx.replyWithHTML('Your invite is published, liveTime of invite 30 min, your can delete it prematurely',
		Extra
			.HTML()
			.markup((markup) => markup.resize()
				.keyboard([
	...
	if (friends.length === 0) {
		return ctx.replyWithHTML('I did not find invitations nearby. Just wait.');
	}
